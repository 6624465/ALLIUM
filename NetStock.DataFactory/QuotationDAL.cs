...
namespace NetStock.DataFactory
{
    public class QuotationDAL
    {
        ...
        public QuotationPricesWithCustomerandProduct GetQuotationItem(string customerCode, string productCode)
        {
            try
            {
                var quotationItem = db.ExecuteSprocAccessor(DBRoutine.SPECIFICPRODUCTSELLRATEFORCUSTOMER,
                                                   MapBuilder<QuotationPricesWithCustomerandProduct>
                                                   .BuildAllProperties(),
                                                  customerCode, productCode).FirstOrDefault();
