...
namespace NetStock.DataFactory
{
    public class GoodsIssueDAL  
    {
        ...
        public List<GoodsIssue> GetList(short branchID)
        {
            return db.ExecuteSprocAccessor(DBRoutine.LISTGOODSISSUE, MapBuilder<GoodsIssue>.MapAllProperties().DoNotMap(x => x.GoodsIssueDetails).Build(), branchID).ToList();
        }
